 ...
 */
function loadPostGraduteKeyan(){
    var saveUrl = "/teachers/student/addPostGraduateKeyan.do";
    var deleteUrl = "/teachers/student/deletePostGraduateKeyan.do";
    var updateUrl = "/teachers/student/editPostGraduateKeyan.do";
    var loadDataUrl = "/teachers/student/loadPostGraduateKeyan.do";
    $.jgrid.defaults.styleUI = 'Bootstrap';
    var lastFlag;
    $("#tPostGraduateKeyanList").jqGrid({
        url: loadDataUrl,
        datatype: "json",
        height: 170,
        editable: true,
        ...
        // caption: "学生记录",
        pager: "#pagerTPostGraduateKeyanList",
        add: true,
        edit: true,
        addtext: 'Add',
        edittext: 'Edit',
    ...
    // Add selection
    $("#tPostGraduateKeyanList").setSelection(4, true);
    // Setup buttons
    $("#tPostGraduateKeyanList").jqGrid('navGrid', '#pagerTPostGraduateKeyanList',{
            edit: true,
            add: true,
            del: true,
            search: true,
    ...
    $(window).bind('resize', function () {
        var width = $('.tPostGraduteKeyan_wrapper').width();
        $('#tPostGraduateKeyanList').setGridWidth(width);
    });
}
...
function uploadPostGraduateKeyan() {
    $.ajax({
        type:'post',
        url:'/teachers/student/requestUploadPostGraduateKeyan.do',
        data: "",
        dataType : 'text',
        success:function(data){
            if(data.toString()=="success"){
                window.location.href = "/teachers/student/downloadExcelPostGraduateKeyan.do";
            }else{
                alert("服务器出错，无法下载")
            }
...
function downPostGraduateKeyan(){
    window.location.href = "/teachers/fileTemplates/postGraduateKeyan.xlsx";
}


function inPostGraduateKeyan() {
    //清空form弹窗内内容
    $("#addFormContent").html("");
    ...
    // 设置点击保存按钮后触发的事件,此处可以写成路径地址
    // 设置点击保存按钮后触发的事件,此处可以写成路径地址
    $("#addForm").attr("onsubmit", 'savePostGraduateKeyan()');
}


function savePostGraduateKeyan() {
    ...
    $.ajax({
        type: 'post',
        url: '/teachers/student/inPostGraduateKeyan.do',
        async: false,
        xhrFields: {
            withCredentials: true
        },
