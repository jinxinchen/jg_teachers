...
function downLoadtExcel() {
    $.ajax({
        type:'post',
        url:'/teachers/scientificResearchMaterials/requestUpload.do',
        data: "",
        dataType : 'text',
        success:function(data){
            if(data.toString()=="success"){
                window.location.href = "/teachers/scientificResearchMaterials/downloadExcel.do";
            }else{
                alert("服务器出错，无法下载")
            }
...
function loadScientificResearchMaterials(){
    var saveUrl = "/teachers/scientificResearchMaterials/addMaterial.do";
    var deleteUrl = "/teachers/scientificResearchMaterials/deleteMaterial.do";
    var updateUrl = "/teachers/scientificResearchMaterials/editMaterial.do";
    var loadDataUrl = "/teachers/scientificResearchMaterials/loadMaterial.do";
    $.jgrid.defaults.styleUI = 'Bootstrap';
    var lastFlag;
    $("#tScientificResearchMaterialsList").jqGrid({
        url: loadDataUrl,
        datatype: "json",
        height: 170,
        editable: true,
        ...
        rowNum: 4,
        rowList: [5, 10, 15],
        colNames: ['材料名','附件上传','附件下载','上传时间','操作'],
        colModel: [
            {
                name:'name',
                index:'name',
        ...
        // caption: "学生记录",
        pager: "#pagerTScientificResearchMaterialsList",
        add: true,
        edit: true,
        addtext: 'Add',
        edittext: 'Edit',
    ...
    // Add selection
    $("#tScientificResearchMaterialsList").setSelection(4, true);
    // Setup buttons
    $("#tScientificResearchMaterialsList").jqGrid('navGrid', '#pagerTScientificResearchMaterialsList', {
        edit: true,
        add: true,
        del: true,
        search: true,
    ...
    $(window).bind('resize', function () {
        var width = $('.tScientificResearchMaterials_wrapper').width();
        $('#tScientificResearchMaterialsList').setGridWidth(width);
    });
}
...
function saveFile() {
    ...
    $.ajax({
        type: 'post',
        url: 'http://192.168.213.253:8099/teachers_files/scientificResearchMaterials/uploadMaterial.do',
        async: false,
        xhrFields: {
            withCredentials: true
        },
...
function downLoadFile(id){
    $.ajax({
        type: 'post',
        url: '/teachers/scientificResearchMaterials/getMaterialSrcById.do',
        // async: false,
        xhrFields: {
            withCredentials: true
        },
