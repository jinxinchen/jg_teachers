...
public class EduController extends test2 {
    ...
    public void requestExcelData(HttpServletRequest request,HttpServletResponse response) throws IOException,WriteException {
        ...
        String strFileName="education.xls";// 默认Excel名称
        ...
        eduService.getOutStream(os,condition);
        os.close();
        os.flush();
    ...
    Page<TEducationExperienceEntity> loadEduExp(HttpServletRequest request, Page<TEducationExperienceEntity> page,String userId){
        ...
        if(StringUtils.isEmpty(userId)){
            request.getSession().setAttribute("educationTeacherCondition",page.getFilterModel());//为了下载做准备
    ...
    @ResponseBody
    Map addEduExp(HttpServletRequest request, String userId,String school, String major, String education,String entrance ,String graduationYear,String abroad){
        Map map = new HashMap();
        int user_id;
        if(StringUtils.isEmpty(userId)){
            user_id = (int) request.getSession().getAttribute("user_id");
        ...
        int status = eduService.addEduExp(user_id,school,major,education,entrance,graduationYear,abroad);
        if(status == 200){
            map.put("status",200);
            return map;
        }else{
    ...
    @ResponseBody
    Map editEduExp(HttpServletRequest request,String userId, int id, String school, String major, String abroad,String education,String entrance ,String graduationYear){
        Map map = new HashMap();
        int user_id;
        if(StringUtils.isEmpty(userId)){
            user_id = (int) request.getSession().getAttribute("user_id");
        ...
        int status = eduService.editEduExp(id,user_id,school,major,education,entrance,graduationYear,abroad);
        if(status == 200){
            map.put("status",200);
            return map;
        }else{
    ...
    @ResponseBody
    ResponseWrapper deleteEduExp(HttpServletRequest request, String id){
        ...
        String[] ids=id.split(",");     try {
            for (int i=0;i<ids.length;i++){
                int status = eduService.deleteEduExp(Integer.valueOf(ids[i].trim()));
            }
            wrapper.setSuccess(true);
        }catch (Exception ex){
            ex.printStackTrace();
